void RDF_thrown(Int_t n_threads,string inf_name, string opf_name, Bool_t show_cut_report) {
	...
	//4.1) Histograms
	auto im_kskl = rdf_cut.Filter("mandel_t > 0.1 && mandel_t < 0.5").Histo1D({"im_kskl2", ";M(K_{S}K_{L});Counts / 1 MeV",  75, 0.98, 2.48}, "mkskl");
	auto im_kskl2 = rdf_cut.Histo1D({"im_kskl", ";M(K_{S}K_{L});Counts / 1 MeV",  110, 0.99, 1.10}, "mkskl");

	auto h1_t = rdf_cut.Histo1D({"h1_mandelt", ";M(K_{S}K_{L});Counts / 1 MeV", 75, 0, 1.5}, "mandel_t");
