...
void RDF_ana(Int_t n_threads,string inf_name, string opf_name, Bool_t show_cut_report) {	
	...
	auto rdfChisqNdf = rdf_variables.Filter(cuts);
	...
	auto h1_chisqNdf = rdfChisqNdf.Filter(signal).Histo1D({"h1_chisqNdf", ";#chi^{2}/ndf;Counts", 200, 0, 20.0}, "chisq_ndf", "accidental_weight");
