...
void DSelector_phi::Init(TTree *locTree)
{
	...
	*/
	...
	if(dIsMC) {
		...
		dFlatTreeInterface->Create_Branch_Fundamental<double>("pippim_res");
		dFlatTreeInterface->Create_Branch_Fundamental<double>("recoilp_res");
...
Bool_t DSelector_phi::Process(Long64_t locEntry)
{
	...
	*/
	...
	for(UInt_t loc_i = 0; loc_i < Get_NumCombos(); ++loc_i)
	{
		...
		TLorentzVector locDecayingKShortX4 = locPiPlusX4 + locPiMinusX4;

		TLorentzVector locProtonX4_Thrown;
		TLorentzVector locDecayingKShortX4_Thrown;
		TLorentzVector locPiPlusX4_Thrown;
		...
		for(UInt_t loc_i = 0; loc_i < Get_NumThrown(); ++loc_i)
		{
			...
			if(locPID == 14) {
				...
				locPiPlusX4_Thrown = locThrownX4;
			}
		}
		...
		if(dIsMC) {
			...
			// dFlatTreeInterface->Fill_Fundamental<double>("pippim_res", (locKShort_X4.Vect().Mag() - locDecayingKShortX4_Thrown.Vect().Mag()));
			dFlatTreeInterface->Fill_Fundamental<double>("pippim_res", (locKShort_X4.Vect().Mag() - locPiPlusX4_Thrown.Vect().Mag()));
			dFlatTreeInterface->Fill_Fundamental<double>("recoilp_res", (locProtonX4.Vect().Mag() - locProtonX4_Thrown.Vect().Mag()));

			cout << "recon vertex: " << locKShort_X4.Vect().Mag() << "	thrown vertex:" << locDecayingKShortX4_Thrown.Vect().Mag() << endl;
...
//		dFlatTreeInterface->Fill_TObject<TLorentzVector>("pim_p4_meas", locPiMinusP4_Measured);
		...
		//FILL FLAT TREE
		if(locBeamP4.E() > 8.2 && locBeamP4.E() < 8.8 && locKSKL_P4.M() < 1.1 && t < 1.5)
			Fill_FlatTree();
		else {
			dComboWrapper->Set_IsComboCut(true);
			continue;
