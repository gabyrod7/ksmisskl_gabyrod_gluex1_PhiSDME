...
void high_low_mkskl() {
	...
	inf[1] = TFile::Open("../mkskl_low/sdme.root");
	inf[2] = TFile::Open("../mkskl_high/sdme.root");
...
//	g[3][7]->Draw("SAME P");
	...
	g[1][8]->Draw("SAME P");
	g[2][8]->Draw("SAME P");
	cout << g[0][8]->GetPointY(0) << "	" <<  g[0][8]->GetErrorY(0) << endl;
	cout << g[1][8]->GetPointY(0) << "	" <<  g[1][8]->GetErrorY(0) << endl;
	cout << g[2][8]->GetPointY(0) << "	" <<  g[2][8]->GetErrorY(0) << endl;
//	g[3][8]->Draw("SAME P");

	TLegend *lg = new TLegend(0.13, 0.17, 0.98, 0.37);
	lg->SetNColumns(2);
	lg->AddEntry(g[0][0], "1.005 < M(K_{S}K_{L}) < 1.050 GeV", "f");
	lg->AddEntry(g[1][0], "1.005 < M(K_{S}K_{L}) < 1.019 GeV", "lep");
	lg->AddEntry(g[2][0], "1.019 < M(K_{S}K_{L}) < 1.050 GeV", "lep");
//	lg->AddEntry(g[3][0], "1.000 < M(K_{S}K_{L}) < 1.060 GeV", "lep");

	c->cd(2);
	lg->Draw();

	c->SaveAs("sdme_high_low_mkskl.png");
	c->SaveAs("sdme_high_low_mkskl.pdf");
	TFile *opf = TFile::Open("sdme_high_low_mkskl.root", "RECREATE");
	c->Write();
...
//		c->cd(i+1);
//		g[1][i]->GetYaxis()->SetRangeUser(-20, 20);
//		g[1][i]->Draw( "AP" );
//		g[2][i]->Draw( "P SAME" );
////		g[3][i]->Draw( "P SAME" );
...
//		line->DrawLine(0.15, -4, 1.5, -4);
//	}
//	c->SaveAs("barlow_high_low_mkskl.pdf");
}
