...
void tmp() {
	...
	inf[1] = TFile::Open("jpac_model.root");
	...
	for(int i = 0; i < 2; i++)
		for(int j = 0; j < 9; j++) {
			...
			if(i == 0)	set_marker(g[i][j], 20, 1);	// kskl channel
			if(i == 1)	set_line(g[i][j], 2, 4, 3);	// jpac model
		}
	...
	lg->SetNColumns(4);
	lg->AddEntry(g[0][0], "K_{S}K_{L}", "lep");
//	lg->AddEntry(g[1][0], "K^{+}K^{-}", "lep");
	lg->AddEntry(g[1][0], "JPAC Model", "lp");
