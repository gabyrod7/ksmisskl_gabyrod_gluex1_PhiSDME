...
//	gStyle->SetMarkerStyle(8);
	...
	TFile *inf[2];
	inf[0] = TFile::Open("../april_10/sdme_nominal.root");
	inf[1] = TFile::Open("../april_10/sdme_bootstrap.root");
...
//			else		g[i][j]->Draw("P SAME");
	...
	c->cd(1);
	draw_graph(g[0][0], "#rho_{00}^{0}",  0.01, 1.50, -0.25, 0.25, kTRUE, 0.00);
	g[1][0]->Draw("SAME P");

	c->cd(2);
	draw_graph(g[0][1], "Re(#rho_{10}^{0})",  0.01, 1.50, -0.25, 0.25, kTRUE, 0.00);
	g[1][1]->Draw("SAME P");

	c->cd(3);
	draw_graph(g[0][2], "#rho_{1-1}^{0}", 0.01, 1.50, -0.25, 0.25, kTRUE, 0.00);
	g[1][2]->Draw("SAME P");

	c->cd(4);
	draw_graph(g[0][3], "#rho_{11}^{1}",  0.01, 1.50, -0.25, 0.25, kTRUE, 0.00);
	g[1][3]->Draw("SAME P");

	c->cd(5);
	draw_graph(g[0][4], "#rho_{00}^{1}",  0.01, 1.50, -0.25, 0.25, kTRUE, 0.00);
	g[1][4]->Draw("SAME P");

	c->cd(6);
	draw_graph(g[0][5], "Re(#rho_{10}^{1})",  0.01, 1.50, -0.25, 0.25, kTRUE, 0.00);
	g[1][5]->Draw("SAME P");
	...
	c->cd(8);
	draw_graph(g[0][7], "Im(#rho_{10}^{2})",  0.01, 1.50, -0.25, 0.25, kTRUE, 0.00);
	g[1][7]->Draw("SAME P");
	...
	lg->SetNColumns(4);
	lg->AddEntry(g[0][0], "Nominal fit", "lep");
	lg->AddEntry(g[1][0], "Bootstrap fit", "lep");
	...
	c->SaveAs("sdme_bootstrap.png");
	c->SaveAs("sdme_bootstrap.pdf");
}
