...
void diff_pol_angles() {
	...
	inf[1] = TFile::Open("../diff_pol_angles/9bins_1.50t_000.root");
	inf[2] = TFile::Open("../diff_pol_angles/9bins_1.50t_045.root");
	inf[3] = TFile::Open("../diff_pol_angles/9bins_1.50t_090.root");
	inf[4] = TFile::Open("../diff_pol_angles/9bins_1.50t_135.root");
	...
	lg->AddEntry(g[1][0], "#Phi = 0  ", "lep");
	lg->AddEntry(g[2][0], "#Phi = 45 ", "lep");
	lg->AddEntry(g[3][0], "#Phi = 90 ", "lep");
	lg->AddEntry(g[4][0], "#Phi = -45", "lep");

	c->cd(2);
	lg->Draw();

	c->SaveAs("sdme_DiffPolAngles.png");
	c->SaveAs("sdme_DiffPolAngles.pdf");

	TFile *opf = TFile::Open("sdme_DiffPolAngles.root", "RECREATE");

	TLine *line = new TLine();
	for(int i = 0; i < 9; i++) {
		calc_barlow(g[0][i], g[1][i]);
		calc_barlow(g[0][i], g[2][i]);
		calc_barlow(g[0][i], g[3][i]);
		calc_barlow(g[0][i], g[4][i]);
		c->cd(i+1);
		g[1][i]->GetYaxis()->SetRangeUser(-15, 15);
		g[1][i]->Draw( "AP" );
		g[2][i]->Draw( "P SAME" );
		g[3][i]->Draw( "P SAME" );
		g[4][i]->Draw( "P SAME" );
		...
		line->DrawLine(0.15, -4, 1.5, -4);
		text.DrawLatex(1.1, 10 , SDMEs[names[i]].c_str());
	}

	c->cd(2);
	lg->Draw();

	c->SaveAs("barlow_DiffPolAngles.pdf");
	c->Write();
}
