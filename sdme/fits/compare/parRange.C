...
void parRange() {
	...
	TFile *inf[4];
	inf[0] = TFile::Open("../sdme_scripts/9bins_1.50t.root");
	inf[1] = TFile::Open("../sdme_scripts/9bins_1.50t_2.root");
	inf[2] = TFile::Open("../sdme_scripts/9bins_1.50t_3.root");
	inf[3] = TFile::Open("../sdme_scripts/9bins_1.50t_4.root");
	...
	for(int i = 0; i < 4; i++)
		for(int j = 0; j < 9; j++) {
			...
			if(i == 3)	x_shift_graph(g[i][j], -0.035);
		}
	...
	lg->SetNColumns(2);
	lg->AddEntry(g[1][0], "No parRange", "lep");
	lg->AddEntry(g[0][0], "parRange 1", "lep");
	lg->AddEntry(g[2][0], "parRange 2", "lep");
	lg->AddEntry(g[3][0], "parRange 3", "lep");

	c->cd(2);
	lg->Draw();

	c->SaveAs("sdme_parRange.png");
}
