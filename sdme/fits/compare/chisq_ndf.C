...
void chisq_ndf() {
	...
	inf[1] = TFile::Open("../chisq3/sdme.root");
	inf[2] = TFile::Open("../chisq4/sdme.root");
	inf[3] = TFile::Open("../chisq5/sdme.root");
	...
	lg->SetNColumns(2);
	lg->AddEntry(g[0][0], "#chi^{2}/ndf < 2", "f");
	lg->AddEntry(g[1][0], "#chi^{2}/ndf < 3", "lep");
	lg->AddEntry(g[2][0], "#chi^{2}/ndf < 4", "lep");
	lg->AddEntry(g[3][0], "#chi^{2}/ndf < 5", "lep");

	c->cd(2);
	lg->Draw();

	c->SaveAs("sdme_ChiSqNdf.png");
	c->SaveAs("sdme_ChiSqNdf.pdf");
	TFile *opf = TFile::Open("sdme_ChiSqNdf.root", "RECREATE");
	c->Write();
	...
	for(int i = 0; i < 9; i++) {
		...
		c->cd(i+1);
		g[1][i]->GetYaxis()->SetRangeUser(-10, 10);
		g[1][i]->Draw( "AP" );
		g[2][i]->Draw( "P SAME" );
		g[3][i]->Draw( "P SAME" );
		line->DrawLine(0.15, 4, 1.5, 4);
		line->DrawLine(0.15, -4, 1.5, -4);
		text.DrawLatex(1.1, 5  , SDMEs[names[i]].c_str());
	}
	c->SaveAs("barlow_ChiSqNdf.pdf");
}
