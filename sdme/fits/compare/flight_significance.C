...
void flight_significance() {
	...
	inf[1] = TFile::Open("../fs3/sdme.root");
	inf[2] = TFile::Open("../fs4/sdme.root");
	inf[3] = TFile::Open("../fs5/sdme.root");
	...
	lg->SetNColumns(2);
	lg->AddEntry(g[0][0], "FS > 6", "f");
	lg->AddEntry(g[1][0], "FS > 3", "lep");
	lg->AddEntry(g[2][0], "FS > 4", "lep");
	lg->AddEntry(g[3][0], "FS > 5", "lep");

	c->cd(2);
	lg->Draw();

	c->SaveAs("sdme_FlightSignificance.png");
	c->SaveAs("sdme_FlightSignificance.pdf");
	TFile *opf = TFile::Open("sdme_FlightSignificance.root", "RECREATE");
	c->Write();
	...
	c->SaveAs("barlow_FlightSignificance.pdf");
}
