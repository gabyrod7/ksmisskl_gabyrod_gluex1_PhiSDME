...
//void calc_barlow(TGraphErrors *nominal, TGraphErrors *variation);
...
void missing_mass() {
	...
	inf[1] = TFile::Open("../mmiss1/sdme.root");
	inf[2] = TFile::Open("../mmiss2/sdme.root");
	inf[3] = TFile::Open("../mmiss3/sdme.root");
	...
	lg->SetNColumns(2);
	lg->AddEntry(g[0][0], "0.30 < MM < 0.70 GeV", "f");
	lg->AddEntry(g[1][0], "0.45 < MM < 0.55 GeV", "lep");
	lg->AddEntry(g[2][0], "0.40 < MM < 0.60 GeV", "lep");
	lg->AddEntry(g[3][0], "0.20 < MM < 0.80 GeV", "lep");

	c->cd(2);
	lg->Draw();

	c->SaveAs("sdme_mmiss.png");
	c->SaveAs("sdme_mmiss.pdf");
	TFile *opf = TFile::Open("sdme_mmiss.root", "RECREATE");
	c->Write();
