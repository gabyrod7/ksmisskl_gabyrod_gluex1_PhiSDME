...
void DSelector_phi::Init(TTree *locTree)
{
	...
	*/
	...
	if(dIsMC) {
		...
		dFlatTreeInterface->Create_Branch_Fundamental<double>("beam_energy_thrown");

		dFlatTreeInterface->Create_Branch_NoSplitTObject<TLorentzVector>("recoil");
		dFlatTreeInterface->Create_Branch_NoSplitTObject<TLorentzVector>("ks_p4_thrown");
		dFlatTreeInterface->Create_Branch_NoSplitTObject<TLorentzVector>("kl_p4_thrown");
	}
...
Bool_t DSelector_phi::Process(Long64_t locEntry)
{
	...
	*/
	...
	for(UInt_t loc_i = 0; loc_i < Get_NumCombos(); ++loc_i)
	{
		...
		TLorentzVector locBeamP4Thrown, locProtonP4Thrown, locDecayingKShortP4Thrown, locMissingKLongP4Thrown;
		if(dIsMC) {
			for(UInt_t loc_i = 0; loc_i < Get_NumThrown(); ++loc_i)
			{
				...
				// cout << dThrownBeam->Get_P4().E() << "	" << locBeamP4.E() << endl;

				if(locPID == 1) {
					locBeamP4Thrown = locThrownP4;
					// locBeamX4Thrown = locThrownX4;
				}
				if(locPID == 14) {
					locProtonP4Thrown = locThrownP4;
				}
				else if(locPID == 16) {
					locDecayingKShortP4Thrown = locThrownP4;
				}
				else if(locPID == 10) {
					locMissingKLongP4Thrown = locThrownP4;
				}
			}

			dFlatTreeInterface->Fill_Fundamental<bool>("dIsMC", dIsMC);

			dFlatTreeInterface->Fill_Fundamental<double>("beam_energy_thrown", locBeamP4Thrown.E());

			dFlatTreeInterface->Fill_TObject<TLorentzVector>("recoil", locProtonP4Thrown);
			dFlatTreeInterface->Fill_TObject<TLorentzVector>("ks_p4_thrown", locDecayingKShortP4Thrown);
			dFlatTreeInterface->Fill_TObject<TLorentzVector>("kl_p4_thrown", locMissingKLongP4Thrown);
		}
...
//		dFlatTreeInterface->Fill_TObject<TLorentzVector>("pim_p4_meas", locPiMinusP4_Measured);
		...
		// locFinalStateP4.push_back(locProtonP4Thrown);
		// locFinalStateP4.push_back(locDecayingKShortP4Thrown);
		// locFinalStateP4.push_back(locMissingKLongP4Thrown);
		...
		//FILL FLAT TREE
		if(locBeamP4.E() > 8.0 && locBeamP4.E() < 8.8 && locKSKL_P4.M() < 1.1 && chisq_ndf < 6 && t < 1.5)
			Fill_FlatTree();
		else {
			dComboWrapper->Set_IsComboCut(true);
			continue;
